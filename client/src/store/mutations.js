...
export default {
    ...
    [CLEAR_CURRENT_POST_SOCKET](state) {
        if(state[CURRENT_POST_SOCKET]) {
            state[CURRENT_POST_SOCKET].close()
            state[CURRENT_POST_SOCKET] = null
        }
    ...
    [APPEND_MAIN_WALL_POSTS](state, {posts}) {
        state[MAIN_WALL_POSTS] = [
            ...state[MAIN_WALL_POSTS],
            ...posts
        ]
    ...
    [ADD_PRIVATE_WALL_SOCKET](state, {user:{_id}, socket}){
        if(state[CURRENT_USER]._id===_id) {
            state[CURRENT_USER_SOCKETS].private = socket

        } else {
            state[MAIN_WALL_POSTS_SOCKETS][_id] = socket
        }
    },
    [CLEAR_PRIVATE_WALL_SOCKET](state, {user:{_id}}) {
        if(state[CURRENT_USER]._id===_id) {
            const sockets = state[CURRENT_USER_SOCKETS]
            if(sockets.private){
                sockets.private.close()
